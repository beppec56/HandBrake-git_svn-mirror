...
class Project( Action ):
    ...
    def _action( self ):
        ...
        if repo.type == 'release':
            ...
            self.version = '%s' % (repo.rev)
            url_ctype = '_unstable'
            url_ntype = 'unstable'
            self.build = time.strftime('%Y%m%d') + '01'
            self.title = '%s %s (%s)' % (self.name,repo.rev,self.build)
        else:
            self.version = '%s-%s x264-r2397-aff928d' % (repo.branch,repo.rev)
            url_ctype = '_unofficial'
            url_ntype = 'unofficial'
            self.build = time.strftime('%Y%m%d') + '99'
            self.title = '%s %s-%s (%s) x264-r2397-aff928d' % (self.name,repo.branch,repo.rev,self.build)
