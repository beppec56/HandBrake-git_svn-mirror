...
class Project( Action ):
    ...
    def _action( self ):
        ...
        if repo.type == 'release':
            ...
            self.title = '%s %s (%s) x264-r2555-0c21480' % (self.name,repo.rev,self.build)
        else:
            self.version = '%s-%s' % (repo.branch,repo.rev)
            url_ctype = '_unofficial'
            url_ntype = 'unofficial'
            self.build = time.strftime('%Y%m%d') + '00'
            self.title = '%s %s-%s (%s) x264-r2555-0c21480' % (self.name,repo.branch,repo.rev,self.build)
